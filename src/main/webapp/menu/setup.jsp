...
<ul class="nav">
    ...
    <li id="smtp-li"><a href="javascript:void(0);"><i
                class="glyphicon glyphicon-home"></i>&nbsp;<i
                class="fa fa-chevron-right"></i> SMTP Setup</a></li>
...
<div class="modal fade" id="modal-smtpsetup" tabindex="-1" role="dialog" aria-labelledby="myModalLabelSmtpsetup" aria-hidden="true">
    <form role="form" id="form-smtpsetup">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-footer">
                    <a href="#" class="btn btn-warning" data-dismiss="modal">Cancel</a>
                    <input class="btn btn-primary" type="submit"
                           value="Save Details" id="btn-smtpsetup">
                </div>
            </div>
        </div>
    </form>
        ...
        $('#form-social').bootstrapValidator({
            ...
            fields: {
                ...
                .on('success.form.bv', function (e) {
                    ...
                    var btn = "btn-smtpsetup";
                    var form = "form-smtpsetup";
                    var modal = "modal-smtpsetup";
                    var btn_text = $('#' + btn).val();
                       ...
                       data: {smtpHost: $('#smtpHost').val(), smtpPort: $('#smtpPort').val(), smtpUsername: $('#smtpUsername').val(), smtpPassword: $('#smtpPassword').val()},
                        ...
                        success: function (json) {
                            ...
                            if (json.success)
                            {
                                $('#' + form)[0].reset();
                                $('#' + modal).modal('hide');
                                html = 'Smtp details successfully saved';
                            } else
                                html = 'smtp details could not be saved';
                            bootbox.alert(html);
                            $('#' + btn).val(btn_text);
                        }
                    });
