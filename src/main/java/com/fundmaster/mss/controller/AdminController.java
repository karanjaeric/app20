...
public class AdminController  extends HttpServlet implements Serializable {
				...
				else
				{
						...
						try {
							...
							else
							{
								for (Scheme scheme : schemes) {
									if (session.getAttribute(Constants.SCHEME_ID).equals(String.valueOf(scheme.getId()))) {
					...
					else if(session.getAttribute(Constants.SCHEME_ID) != null)
					{
						for(Scheme scheme : schemes != null ? schemes : null)
						{
							...
							if(scheme.getId().equals(Long.valueOf(session.getAttribute(Constants.SCHEME_ID).toString())))
							{
								...
								try {
									...
									else
									{
										for (Scheme scheme1 : schemes) {
											if (session.getAttribute(Constants.SCHEME_ID).equals(String.valueOf(scheme1.getId()))) {
	...
	protected void doPost(HttpServletRequest request,
			...
			{
				JSONObject result = helper.searchProfilesJSON(request.getParameter("search"), request.getParameter("identifier"), request.getParameter("profile"), session.getAttribute(Constants.SCHEME_ID).toString());
			...
			{
				User u = userEJB.find(session.getAttribute(Constants.USER).toString(), session.getAttribute(Constants.U_PROFILE).toString());
				...
				try {
					JSONObject resp = helper.sendNotification(m.getEmailAddress(), "Change Password Request", "Dear " + u.getUsername() + ", " +
