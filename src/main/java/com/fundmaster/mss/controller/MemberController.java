...
import com.fundmaster.mss.dto.MemberBenefitDetailsDTO;
        ...
        try {
            if (session != null) {
                if (!(session.getAttribute(Constants.LOGIN).equals(true) && (this.getSessKey(request, Constants.U_PROFILE).equals(Constants.MEMBER_PROFILE)
                    ...
                    MssUtil.castToBigDecimal(10);
                    memberBenefitsDto.setEmployee(MssUtil.castToBigDecimal(obj.get("ee")));
                    memberBenefitsDto.setEmployer(MssUtil.castToBigDecimal(obj.get("er")));
                    memberBenefitsDto.setAvc(MssUtil.castToBigDecimal(obj.get("avc")));
                    memberBenefitsDto.setInterest(MssUtil.castToBigDecimal(obj.get("interest")));
                    memberBenefitsDto.setGrandTotal(MssUtil.castToBigDecimal(obj.get("grandTotal")));
